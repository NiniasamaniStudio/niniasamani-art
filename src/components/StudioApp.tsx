...
function ProductCard({ product, index, onSelect }: { product: Product; index: number; onSelect: () => void }) {
  return <motion.article className={`product-card tone-${product.tone}`} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96 }} transition={{ delay: index * 0.04 }}><button className="product-image" onClick={onSelect} aria-label={`${product.title} დეტალები`}><Image src={product.image} alt={product.title} fill sizes="(max-width: 430px) 88vw, (max-width: 800px) 44vw, 30vw" /><span className="product-plus"><Plus size={18} /></span></button><div className="product-info"><div><span className="product-category">{product.categoryLabel}</span><h3>{product.title}</h3></div><span className="product-price">{product.price}</span></div><div className="product-bottom"><div className="product-tags">{product.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><button onClick={() => window.open(whatsappLink(`გამარჯობა, მაინტერესებს ნამუშევარი: ${product.title}`), "_blank")} className="inquire">მოკითხვა <ArrowUpRight size={14} /></button></div></motion.article>;
}

function ProductModal({ product, onClose }: { product: Product; onClose: () => void }) { return <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}><motion.div className="detail-modal" initial={{ y: 30 }} animate={{ y: 0 }} exit={{ y: 30 }} onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose} aria-label="დახურვა"><X size={20} /></button><Image src={product.image} alt={product.title} width={800} height={800} /><div className="modal-copy"><span className="product-category">{product.categoryLabel}</span><h2>{product.title}</h2><p>{product.description}</p><strong>{product.price}</strong><a className="button button-dark" href={whatsappLink(`გამარჯობა, მაინტერესებს ${product.title}. გთხოვთ, მომწეროთ დეტალები.`)} target="_blank" rel="noreferrer">დაკავშირება WhatsApp-ზე <ArrowUpRight size={16} /></a></div></motion.div></motion.div>; }
