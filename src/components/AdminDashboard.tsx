...
export default function AdminDashboard() {
  ...
  return <AdminFrame><div className="admin-dashboard"><header className="admin-topbar"><div><p className="section-kicker">/ content room</p><h1>სტუდიის<br /><em>კოლექცია.</em></h1></div><div className="admin-actions"><span>{sessionEmail}</span><button className="admin-icon-button" onClick={signOut} aria-label="გასვლა"><LogOut size={18} /></button></div></header><div className="admin-layout"><form className="admin-editor" onSubmit={saveProduct}><div className="admin-form-heading"><div><span className="admin-label">{editingId ? "რედაქტირება" : "ახალი ნამუშევარი"}</span><h2>{editingId ? "განაახლე დეტალები" : "დაამატე კოლექციას"}</h2></div>{editingId && <button type="button" className="admin-icon-button" onClick={resetForm} aria-label="რედაქტირების გაუქმება"><X size={18} /></button>}</div><label>დასახელება<input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} required /></label><div className="admin-field-grid"><label>კატეგორია<select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value as ProductCategory })}>{Object.entries(categoryLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>ფასი<input value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} placeholder="₾ 85-დან" required /></label></div><label>აღწერა<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={4} required /></label><label>ტეგები <span className="admin-hint">მძიმით გამოყავი</span><input value={form.tags} onChange={(event) => setForm({ ...form, tags: event.target.value })} placeholder="მძივები, ხელნაკეთი" /></label><label>სურათის URL<input value={form.image} onChange={(event) => setForm({ ...form, image: event.target.value })} placeholder="https://..." required /></label><label className="admin-upload"><Upload size={16} /> სურათის ატვირთვა<input type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadImage(file); }} /></label><div className="admin-field-grid"><label>ტონი<input value={form.tone} onChange={(event) => setForm({ ...form, tone: event.target.value })} /></label><label>რიგი<input type="number" value={form.sort_order} onChange={(event) => setForm({ ...form, sort_order: Number(event.target.value) })} /></label></div><label className="admin-checkbox"><input type="checkbox" checked={form.published} onChange={(event) => setForm({ ...form, published: event.target.checked })} /> საჯაროდ გამოქვეყნებული</label><button className="button button-dark" disabled={loading}>{editingId ? "შენახვა" : "დამატება"} <Plus size={16} /></button>{notice && <p className="admin-notice">{notice}</p>}</form><section className="admin-list"><div className="admin-list-heading"><div><span className="admin-label">{products.length} ნამუშევარი</span><h2>კოლექცია</h2></div><button className="button button-light" onClick={resetForm}><Plus size={16} /> ახალი</button></div>{products.length === 0 && <div className="admin-empty"><p>Supabase-ში ჯერ არ არის პროდუქტი.</p><span>შეავსე ფორმა ან ჩასვი ჩანაწერები SQL Editor-ით.</span></div>}{products.map((product) => <article className="admin-product" key={product.id}><div className="admin-product-image" style={{ backgroundImage: `url(${product.image})` }} /><div className="admin-product-copy"><span>{product.category_label}</span><h3>{product.title}</h3><p>{product.price} · {product.published ? "გამოქვეყნებული" : "დამალული"}</p></div><div className="admin-product-actions"><button className="admin-icon-button" onClick={() => editProduct(product)} aria-label={`${product.title} რედაქტირება`}><Save size={16} /></button><button className="admin-icon-button danger" onClick={() => void deleteProduct(product.id)} aria-label={`${product.title} წაშლა`}><Trash2 size={16} /></button></div></article>)}</section></div></div></AdminFrame>;
}
